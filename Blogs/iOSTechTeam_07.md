...
要捕获 `@try{}` 块中引发的异常，需要在 `@try{}` 块之后使用一个或多个 `@catch{}` 块。 `@catch{}` 块应按从最具体到最不具体的顺序排列。 这样，我们可以将异常处理定制为组，如下面的一个异常处理程序：
...
重要提示：NSError 类在 OS X 和 iOS 上都可用。但是，错误响应和错误恢复 API 和机制仅在 Application Kit (OS X) 中可用。
